...
import { BillPaySaving } from "./BillPaymentsSaving";
...
export default function Products() {
  ...
  const handlePayClick = (amount) => {
    // Set the amount and show the payment form
    setPaymentDetails({
      ...paymentDetails,
      amount: amount
    });
        ...
        <div className="mt-6 grid grid-cols-1 gap-x-8 gap-y-8 sm:grid-cols-2 sm:gap-y-10 lg:grid-cols-4">
          {products.map((product) => (
            <div key={product.id} className="group relative">
              ...
              <button
                onClick={() => handlePayClick(product.price.replace('$', ''))}
                className="mt-4 w-full bg-indigo-600 text-white px-4 py-2 rounded-md hover:bg-indigo-500"
              >
                Pay ${product.price.replace('$', '')}
              </button>
            </div>
          ))}
        </div>
      </div>
      {showForm && <BillPaySaving amount={paymentDetails.amount} />} {/* Render form conditionally */}
    </div>
  );
}
